            ...
            <div className="w-full !m-0 h-full !max-w-[calc(100vw)] overflow-y-hidden overflow-x-auto !flex items-center rounded-full ">
                <List className="flex-row h-full flex w-full !min-w-max justify-evenly items-center  !p-0  ">
                    {categoriesArray.map((category, index) => (
                        <List.Item className="px-0.5 !py-0 h-full w-full min-w-max hover:!bg-transparent" key={index}>
                            <label htmlFor={category} className="flex flex-1">
                                <List.ItemStart className={` relative w-full !px-0.5 !m-0 flex items-center justify-center rounded-full border-[1px] min-h-[32px]  ${checkedState[index] ? ` ${` ${color}Style !border-none `} ` : ` md3-${color}-outlined  `}`}>
                                    <Checkbox
