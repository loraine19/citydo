...
export default function CTAMines({ disabled1, disabled2, actions }: CTAProps) {
    ...
    return (
        <footer className={`CTA slateFooter `}>
            <div className={`flex gap-x-3 gap-y-2 md:gap-x-6 md:gap-y-3 flex-row flex-wrap items-center justify-center w-full wRespL px-4 lg:px-2 pb-2 lg:pb-0 `}>
                {[...actions]
                    .sort((a, b) => {
                            ...
                            <div key={i}
                                ...
                                <div className={`flex flex-1 bg-slate-100 max-w-[max-content] rounded-full !justify-end items-end   `}>
                                    <button
                                        ...
                                        className={` showUp ${action?.color ?? defColor ?? 'slate'}${(isPrimary(i) && !isDisabled(i)) ? 'StyleInv ' : 'Style'} !min-w-max  lgBtn !mr-0 flex-1 flex !py-1 !px-2 anim`}
                                    >
                                        {action?.iconImage && (
                                            <Icon
                                                onClick={() => {
