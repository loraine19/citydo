                            ...
                            {faqData.map(({ category, color, icon, items }, index) =>
                                <div className={`${openCategoryIndex === index ? "pb-6 " : ""} py-1  px-3 flex- justify-center items-center cursor-pointer`}>
                                    <div onClick={() => {
                                        ...
                                        <h6
                                            key={index}
                                            className={`${openCategoryIndex === index ? `w-full  border-b-[2px] md3-border-${color}` : "underline-offset-0"} -mb-1 font-semibold`}>
                                            {category}
                                        </h6>
                                    </div>
                                    <List className={`${openCategoryIndex === index ? "flex" : " hidden"} px-3 gap-2 flex !rounded-3xl `}>
                                        {items.map(({ question, answer, icon }, index2) =>
                                            <ListItem
                                                className={`!w-full items-start rounded-xl overflow-hidden !flex !flex-1
                                                ...
                                                {<div
                                                    ...
                                                    className='relative flex min-w-max items-center gap-3'>
                                                    <Icon reverse fill={openQuestionIndex === index2 + category} color={color} icon={icon} size='lg' bg />
                                                    <h6>
                                                        {question}
                                                    </h6>
                                                </div>}>
