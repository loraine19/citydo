...
export default function MyInfosPage() {
    ...
    const { groups, error: groupError, isLoading: groupLoading } = DI.resolve('groupViewModel')();

    const [initialValues, setInitialValues] = useState<ProfileDTO>({
        Address: Profile?.Address,
        groups: user?.GroupUser?.map((g: GroupUser) => g?.groupId?.toString()),
        ...Profile
    })
    ...
    }, [Profile.userId, groupLoading, groupError])
